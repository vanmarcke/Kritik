...
class RankingRepository extends ServiceEntityRepository
{
    ...
    public function moyenne($id_record){
        // SELECT AVG(note) as moyenne FROM ranking r WHERE r.record_id = $id_record
        $resultat = $this->createQueryBuilder('r')
                        ->andWhere('r.record = :val')
                        ->setParameter('val', $id_record)
                        ->select("AVG(r.note) as moyenne")
                        ->getQuery()
                        ->getOneOrNullResult()
        ;
        return $resultat ? $resultat["moyenne"] : null;
    }
